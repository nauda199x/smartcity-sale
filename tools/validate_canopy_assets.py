#!/usr/bin/env python3
"""Validate The Canopy Residences dossier and local image pack."""
...
MANIFEST=ROOT/"data"/"official"/"canopy-assets.json"
PAGE=ROOT/"phan-khu-smart-city"/"canopy"/"index.html"
HUB=ROOT/"mat-bang-smart-city"/"canopy"/"index.html"
...
def main():
    ...
    if len(assets)!=9: errors.append(f"expected 9 assets, got {len(assets)}")
    ...
    for req in ("1.758","586","38 tầng","2 tầng","04/07/2025","P17","P18","P19","Newlife","16 căn"):
        if req not in text: errors.append(f"missing fact {req}")
    if "FAQPage" not in html or len(soup.select(".canopy-faq details"))<8: errors.append("FAQ incomplete")
    ...
    if errors:
        print(f"CANOPY VALIDATION FAILED ({len(errors)} errors)")
        for e in errors: print("-",e)
        raise SystemExit(1)
    print(f"CANOPY VALIDATION PASSED: 9 traced WebP files ({dict(mix)}), {words} public words, 9 FAQ items, no public source UI/hotlinks, TC1-TC3 plans local")
